...
public class SpringScimServer implements EmbeddedServerExtension.ScimTestServer {

  private ConfigurableApplicationContext context;

  @Override
  public URI start(int port) {
    context = SpringApplication.run(ScimpleSpringApp.class,
      "--server.servlet.context-path=/v2",
      "--server.port=" + port
    );
    return URI.create("http://localhost:" + port + "/v2");
  }
