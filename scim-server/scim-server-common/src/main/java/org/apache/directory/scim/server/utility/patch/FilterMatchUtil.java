...
class FilterMatchUtil {
    ...
    public static boolean complexAttributeMatch(final Schema.Attribute parent,
        ...
        if (Objects.isNull(valuePathExpression)) {
            throw throwScimException(Response.Status.BAD_REQUEST, ErrorMessageType.INVALID_FILTER);
        }

        if (Objects.isNull(valuePathExpression.getAttributeExpression())) {
            throw throwScimException(Response.Status.BAD_REQUEST, ErrorMessageType.INVALID_FILTER);
        }
    ...
    static boolean comparisonExpression(final AttributeComparisonExpression attributeComparisonExpression,
        ...
        if (!parent.getAttributes().isEmpty() && !Objects.isNull(subAttr)) {
            final Schema.Attribute subAttribute = parent.getAttribute(subAttr);
            if (Objects.nonNull(subAttribute)) {
    ...
    static boolean comparisonExpression(final Schema.Attribute subAttribute,
                                        final Object value,
